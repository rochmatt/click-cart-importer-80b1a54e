...
export function CategoryMegaMenu({ onQueryChange }: CategoryMegaMenuProps) {
  ...
  const onTabKeyDown = (event: React.KeyboardEvent, index: number) => {
    ...
    if (event.key === "ArrowRight") {
      event.preventDefault();
      const panelSelector = `#${CSS.escape(`${baseId}-tabpanel-${slug(categories[activeIndex].label)}`)}`;
  ...
  return (
    ...
    >
      ...
      >
        <div className="grid w-[880px] grid-cols-[210px_1fr_260px] overflow-hidden rounded-2xl border border-border bg-popover shadow-xl">
          <div
            role="tablist"
