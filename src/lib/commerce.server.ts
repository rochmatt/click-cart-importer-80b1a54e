...
// Parameter klien dihapus; fungsi ini membuka koneksinya sendiri. Dulu ia
// menerima klien Supabase dari pemanggil, saat admin_products sudah pindah ke
// PostgreSQL lokal sementara orders belum.
...
export async function createOrder(data: PlaceOrderData): Promise<PlaceOrderResult> {
  ...
  if (userId) await db.from("cart_items").delete().eq("user_id", userId);
  ...
  await db.from("orders").update({ notified_status: "processing" }).eq("id", order.id);
