            ...
            <Tooltip>
              ...
              <TooltipContent side="right" className="max-w-xs">
                <p className="mb-1.5 font-semibold">
                  {expanded ? "Semua item ditampilkan" : `${hiddenLabels.length} item tersembunyi`}
                </p>
                {!expanded ? (
...
export function CategoryFilters({
  ...
  // Tombol "Hapus semua" menonaktifkan dirinya begitu filter nol, sehingga fokus
  // akan jatuh ke <body>. Setelah reset, kembalikan fokus ke kontrol filter
  // pertama (kotak pencarian) supaya pengguna keyboard/screen reader tak tersesat.
  const searchRef = useRef<HTMLInputElement>(null);

  const allCategories = useMemo(() => (groups ?? []).flatMap((g) => g.items), [groups]);

  return (
    <TooltipProvider>
      <section
        aria-labelledby={`${uid}-heading`}
        className={cn("rounded-2xl border border-border bg-card px-4 py-5 sm:px-5", className)}
      >
        {/* Semua Kategori */}
        <h2
          id={`${uid}-heading`}
          ...
          <div className="mt-4 border-t border-border pt-4">
            ...
            {subcategories && subcategories.length > 0 ? (
              <div className="mt-2 pl-4">
                <MoreList label={`subkategori ${activeLabel}`} limit={5} labels={subcategories}>
                    ...
                    return (
                      <li key={sub}>
                        <Link
                          to="/search"
                          search={{ q: sub }}
                          aria-label={n === undefined ? sub : `${sub}, ${n} produk`}
                          className={cn(
        ...
        {allCategories.length > 0 ? (
          <div className="mt-4 border-t border-border pt-4">
            ...
            <MoreList
              label="kategori lain"
              limit={4}
              labels={allCategories.filter((c) => c.slug !== activeSlug).map((c) => c.label)}
            >
              {allCategories
                  ...
                  return (
                    <li key={c.slug}>
                      <Link
                        to="/category/$slug"
                        params={{ slug: c.slug }}
                        aria-label={n === undefined ? c.label : `${c.label}, ${n} produk`}
                        className={cn(
        ...
        <div className="mt-5 border-t border-border pt-5">
          ...
          {/* Ringkasan filter aktif untuk screen reader kini diumumkan dari rute
            kategori (naratif lengkap + jumlah produk), jadi stub hitungan di
            sini dihapus agar tidak dibaca ganda. */}
        ...
        <div className="mt-6 border-t border-border pt-5">
          <button
            ...
            onClick={() => {
              ...
              // Fokus dipindah SEBELUM tombol menonaktifkan diri (count → 0),
              // jadi fokus mendarat di kotak pencarian, bukan hilang ke <body>.
